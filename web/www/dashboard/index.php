...
$db = 'metabrute'; // Replace with your database name
$user = 'metabrute'; // Replace with your database username
$pass = 'password'; // Replace with your database password

// Define the upload directory
$uploadDir = 'uploads/';
$uploadFile = $uploadDir . basename($_FILES['passwordsFile']['name'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $vaultData = $_POST['vaultData'] ?? '';
    $iv = $_POST['iv'] ?? '';
    $salt = $_POST['salt'] ?? '';
    $iterations = $_POST['iterations'] ?? '';

    // Check if the file was uploaded
    if (isset($_FILES['passwordsFile']) && $_FILES['passwordsFile']['error'] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['passwordsFile']['tmp_name'], $uploadFile)) {
            $uploadMessage = "File uploaded successfully!";

            // Read the file content and process passwords
            $passwordsContent = file_get_contents($uploadFile);
            $passwordsArray = explode("\n", trim($passwordsContent));
            $passwordsString = implode('|', array_map('trim', $passwordsArray));

            // Save the form data to the database with passwords processed
            $stmt = $pdo->prepare("INSERT INTO brute (data, iv, salt, iterations, passwords) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$vaultData, $iv, $salt, $iterations, $passwordsString]);

            $uploadMessage .= " Data saved to the database.";

            // Optionally, you could delete the uploaded file after processing
            unlink($uploadFile);
        } else {
            $uploadMessage = "Failed to upload the file.";
        }
    } else {
        $uploadMessage = "No file was uploaded or there was an error with the file upload.";
    }

    // Indicate that the form submission is handled
    $showForm = false;
}
?>
...
<head>
    ...
    <title>MetaBrute</title>
    <style>
    #logo-container {
        position: fixed; /* Use fixed to keep the logo in view even when scrolling */
...
<div class="container">
    ...
    <?php if ($showForm): ?>
        <form id="dataForm" action="" method="post" enctype="multipart/form-data">
            <label for="vaultData">Vault Data:</label>
            <input type="text" id="vaultData" name="vaultData" required><br>

            <label for="iv">IV:</label>
            <input type="text" id="iv" name="iv" required><br>

            <label for="salt">Salt:</label>
            <input type="text" id="salt" name="salt" required><br>

            <label for="iterations">Iterations:</label>
            <input type="text" id="iterations" name="iterations" required><br>

            <label for="passwordsFile">Passwords File (passwords.txt):</label>
            <input type="file" id="passwordsFile" name="passwordsFile" accept=".txt" required><br>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('dataForm').addEventListener('submit', function(event) {
        ...
        .then(() => {
            ...
            // Function to poll for results from get_seed.php
            const checkResponse = () => {
                fetch('get_seed.php', {
                ...
                .then(data => {
                    if (data.trim() !== '') {
                        console.log('Received data from get_seed.php:', data);
                        loadingSpinner.style.display = 'none'; // Hide the loading spinner
                        document.body.insertAdjacentHTML('beforeend', '<p>Response: ' + data + '</p>'); // Show response

                        // Optionally show the submit button again
                        submitButton.style.display = 'inline-block';
                        submitSpinner.style.display = 'none';
                    } else {
                        console.log('No data received, checking again.');
                        setTimeout(checkResponse, 1000); // Check again after 1 second
                    }
                })
                .catch(error => {
                    console.error('Error fetching data from get_seed.php:', error);
